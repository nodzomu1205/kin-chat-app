...
function buildRuleLines(intent: TaskIntent): string[] {
  ...
  if (intent.workflow?.allowYoutubeTranscriptRequest) {
    ...
    lines.push(
      '- When GPT sends transcript parts or queued transcript deliveries, reply each time with <<SYS_KIN_RESPONSE>> Received. Send the next. <<END_SYS_RESPONSE>> so GPT can continue safely.'
    );
    lines.push(
...
BODY: Fetch these transcripts in order. Up to 3 URLs may be requested at once. Each time GPT sends a transcript part or finishes one transcript delivery, reply with <<SYS_KIN_RESPONSE>> Received. Send the next. <<END_SYS_RESPONSE>>.
...
function buildDeliveryLimits(intent: TaskIntent): string[] {
  const lines = [
    ...
    "- If this <<SYS_TASK>> arrives as multiple PART n/total messages, reply only with <<SYS_KIN_RESPONSE>> Received. Send the next. <<END_SYS_RESPONSE>> until the final part arrives.",
