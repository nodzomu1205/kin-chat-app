...
vi.mock("@/lib/app/gptMemoryStorage", async () => {
  const actual = await vi.importActual<typeof import("@/lib/app/gptMemoryStorage")>(
    "@/lib/app/gptMemoryStorage"
  );
  return {
    ...actual,
...
const storageModule = await import("@/lib/app/gptMemoryStorage");
