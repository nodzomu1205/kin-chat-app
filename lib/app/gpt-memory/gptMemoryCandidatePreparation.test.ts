...
vi.mock("@/lib/app/memory-interpreter/memoryInterpreter", () => ({
  resolveMemoryFallbackOptions: (...args: unknown[]) =>
    resolveMemoryFallbackOptionsMock(...args),
}));

vi.mock("@/lib/app/gpt-memory/gptMemoryStateCandidate", () => ({
  buildCandidateMemoryState: (...args: unknown[]) => buildCandidateMemoryStateMock(...args),
}));

vi.mock("@/lib/app/gpt-memory/gptMemorySummarizePolicy", () => ({
