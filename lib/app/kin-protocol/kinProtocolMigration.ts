...
} from "@/lib/app/kin-protocol/kinProtocolText";
import {
...
const LEGACY_PROTOCOL_LIMIT_PATTERN =
  /(?:When GPT sends you a long SYS message, it may split it at 3200-3600 characters and label each part as PART n\/total\. When you send a message out, keep each message at or under 700 characters\. If your message would exceed 700 characters, split it into 600-700 character parts before sending, label each part as PART n\/total, and clearly mark the last part|Keep each message (?:under 3600(?: chars| characters)?\. If (?:over 3200|a message exceeds 3200 characters), split(?: it)?(?: into parts)?, label each part as PART n\/total, and clearly mark (?:the final part|the last part)|at or under 700 characters\. If your message would exceed 700 characters, split it (?:before sending )?into 600-700 character parts before sending, label each part as PART n\/total, and clearly mark the last part|at or under 700 characters\. If a message would exceed 700 characters, split it before sending, label each part as PART n\/total, and clearly mark the last part))\./gi;

const LEGACY_RULEBOOK_LIMIT_PATTERN =
  /- (?:Keep each message (?:under 3600 characters\.\s*- If a message exceeds 3200 characters, split it into parts, label each part as PART n\/total, and clearly mark the final part|at or under 700 characters\.\s*- If a message would exceed 700 characters, split it before sending, label each part as PART n\/total, and clearly mark the last part)|When you send a message out, keep each message at or under 700 characters\.\s*- If your message would exceed 700 characters, split it into 600-700 character parts, label each part as PART n\/total, and clearly mark the last part)\./gi;
...
const UPDATED_RULEBOOK_LIMIT_TEXT =
  `- GPT may send a long SYS message in ${GPT_SYS_MESSAGE_PART_RANGE} character parts labeled as PART n/total.\n- When you send a message out, ${KIN_OUTGOING_MESSAGE_LIMIT_TEXT}.\n- If your message would exceed ${KIN_OUTGOING_MESSAGE_MAX_CHARS} characters, split it into ${KIN_OUTGOING_MESSAGE_PART_MIN_CHARS}-${KIN_OUTGOING_MESSAGE_PART_MAX_CHARS} character parts, label each part as PART n/total, and clearly mark the last part.`;
