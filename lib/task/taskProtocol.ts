...
function buildPresentationPlanTaskPrompt(task: TaskRequest) {
  return [
          ...
          blocks: [
            {
              ...
              visualRequest: {
                ...
                preferredImageId: "Exact Image ID from the provided allowlist when one image is central.",
                candidateImageIds: [
                  "Exact Image ID from the provided allowlist",
                  "Another exact Image ID from the provided allowlist",
                ],
    ...
    "- If image-library candidates are provided, treat their Image IDs as a closed allowlist. visualRequest.preferredImageId and candidateImageIds must use only exact Image IDs from that list; never invent, rename, abbreviate, or derive an Image ID from a filename.",
    "- When image-library candidates are provided and the slide is textMain, list all semantically relevant image-library matches in candidateImageIds in relevance order. Do not decide the final count in the plan; the renderer will choose how many fit.",
    "- For textMain supporting visuals, use usagePolicy = useOneOrMore when several candidates may help, or useAsGrid when the slide message benefits from comparing several images. Leave maxVisualItems unspecified or set it to 3 unless the slide truly has space for only one image.",
    "- When candidateImageIds may render as multiple images, provide visualRequest.labels with one short label per candidate image, matching candidateImageIds exactly: same count, same order, one label per image ID. Do not repeat one generic caption for all images.",
    ...
    "- If image-library candidates are provided but none semantically fit a needed visual, do not invent an ID or a prompt-only placeholder as if it were available. Add the gap to missingInfo or warnings.",
